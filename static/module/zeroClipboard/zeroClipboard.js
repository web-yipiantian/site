  ...
  if (events && events.length) {
    for (i = 0, len = events.length; i < len; i++) {
      ...
      if (perEventHandlers && perEventHandlers.length) {
        if (listener) {
          foundIndex = $.inArray(listener, perEventHandlers);
          while (foundIndex !== -1) {
            perEventHandlers.splice(foundIndex, 1);
            foundIndex = $.inArray(listener, perEventHandlers, foundIndex);
          }
        } else {
...
var _preprocessEvent = function(event) {
  ...
  switch (event.type) {
    case "error":
      if ($.inArray(event.name, flashErrorNames) !== -1) {
        _extend(_flashState, {
...
var _determineScriptAccess = function() {
  ...
  return function(currentDomain, configOptions) {
    ...
    if (len > 0) {
      ...
      if ($.inArray(currentDomain, trustedDomains) !== -1) {
 ...
 * @property {string}
 */
// Hack by WangWenshu, 2014-08-14; Object.defineProperty() method will cause error in some legacy IE browser.
try {
...
var _clientClip = function(elements) {
  ...
  for (var i = 0; i < elements.length; i++) {
    if (_hasOwn.call(elements, i) && elements[i] && elements[i].nodeType === 1) {
      ...
      } else if ($.inArray(this.id, _elementMeta[elements[i].zcClippingId]) === -1) {
        _elementMeta[elements[i].zcClippingId].push(this.id);
      }
      var clippedElements = _clientMeta[this.id] && _clientMeta[this.id].elements;
      if ($.inArray(elements[i], clippedElements) === -1) {
...
var _clientUnclip = function(elements) {
  ...
  for (var i = elements.length; i--;) {
    if (_hasOwn.call(elements, i) && elements[i] && elements[i].nodeType === 1) {
      arrayIndex = 0;
      while ((arrayIndex = $.inArray(elements[i], clippedElements, arrayIndex)) !== -1) {
        clippedElements.splice(arrayIndex, 1);
      }
      var clientIds = _elementMeta[elements[i].zcClippingId];
      if (clientIds) {
        arrayIndex = 0;
        while ((arrayIndex = $.inArray(this.id, clientIds, arrayIndex)) !== -1) {
...
var _clientShouldEmit = function(event) {
  ...
  var goodTarget = !event.target || hasClippedEls && $.inArray(event.target, clippedEls) !== -1;
  var goodRelTarget = event.relatedTarget && hasClippedEls && $.inArray(event.relatedTarget, clippedEls) !== -1;
...
var _addMouseHandlers = function(element) {
  ...
  $(element).on("mouseover", _elementMouseOver);
  $(element).on("mouseout", _suppressMouseEvents);
  $(element).on("mouseenter", _suppressMouseEvents);
  $(element).on("mouseleave", _suppressMouseEvents);
  $(element).on("mousemove", _suppressMouseEvents);
  _mouseHandlers[element.zcClippingId] = {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Set swf path.    By WangWenshu, 2014-08-26
ZeroClipboard.config({
  swfPath: __uri('/static/flash/zeroClipboard.swf')
});
