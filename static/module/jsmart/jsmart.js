...
var buildInFunctions = {
  ...
  foreach: {
    ...
    process: function(node, data) {
      var params = getActualParamValues(node.params, data);
      var a = params.from;
      if (a == undefined){    // 修正循环变量为空时依然输出循环体的 bug.    2014-11-06 by WangWenshu.
        return '';
      } else {
...
var jSmart = function(tpl) {
  ...
  parse(
    // Hack: Remove all modifiers defined by FIS(start with 'f_escape_').    // WangWenshu, 2014-06-13
    applyFilters(jSmart.prototype.filters_global.pre, stripComments((new String(tpl ? tpl : '').replace(/\|f_escape_\w+/g, '')).replace(/\r\n/g, '\n'))),
    this.tree
  );
};
...
jSmart.prototype.getTemplate = function(name) {
  // throw new Error('No template for ' + name);
  return __inline(name); // Overrides by WangWenshu, 2014-06-03, only supporting for fetching files from the local file system.
}
...
jSmart.prototype.left_delimiter = '{%'; // Overrides by WangWenshu, 2014-06-03
jSmart.prototype.right_delimiter = '%}'; // Overrides by WangWenshu, 2014-06-03
...
jSmart.prototype.registerPlugin(
  'modifier',
  'defaultValue',
  function(s, value) {    // 修复 default modifier 无法正确处理传入数据为 0 的情况之问题。 By WangWenshu 2014-12-11.
    return ((s == undefined || s === '' || typeof(s) == 'array' && s.length == 0) ? 
      ((value == undefined || value === '' || typeof(value) == 'array' && s.length == 0) ? '' : value.toString()) : s.toString());
  }
);

jSmart.prototype.registerPlugin(    // Add 'in_array' modifier, By WangWenshu 2015-05-15.
  'modifier',
  'in_array',
  function(s, array) {
    return findInArray(array, s);
  }
);
...
typeof(module) !== 'undefined' && module.exports ? module.exports = jSmart : window.jSmart = jSmart;
